...
const SignUpForm = ({ navigation, route }: AuthStackScreenProps<'SignUp'>) => {
	const INVALID_REPEAT_PASSWORD = 'Пароли не совпадают';

	const [name, setName] = useState<StateConfig>({ value: '', error: '' });
	const [surname, setSurname] = useState<StateConfig>({
		value: '',
		error: '',
	});
	const [password, setPassword] = useState<StateConfig>({
		value: '',
		error: '',
	});
	const [repeatPassword, setRepeatPassword] = useState<StateConfig>({
		value: '',
		error: '',
	});

	const isButtonDisabled =
		Array.of(name, surname, password, repeatPassword).filter(
	...
	const handleChangePasswordRepeat = (value: string) => {
		const error = password.value.localeCompare(value)
	...
	return (
		<Container>
			...
			<View style={styles.main}>
				<Title style={styles.title}>Информация о себе</Title>
			...
			<View style={styles.footer}>
				<Button disabled={isButtonDisabled} onPress={handleSubmit}>
					Зарегистрироваться
				</Button>
			</View>
		</Container>
